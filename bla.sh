    ...
    case $1 in
        en)
            LANG=(
		        ...
		        [TEST_RUN_SUCCESS]="Test run completed successfully"
        ...
        ru)
            LANG=(
		        ...
		        [TEST_MODE_ENABLED]="Включен тестовый режим"
		        [CERT_WILL_NOT_BE_ISSUED]="Реальный сертификат выдан не будет"
		        [TEST_RUN_SUCCESS]="Тестовый запуск успешно завершён"
...
EOL
    ...
    if [ "$CERT_TEST_MODE" = true ]; then
        echo -e "${COLOR_GREEN}✔ ${LANG[TEST_RUN_SUCCESS]}${COLOR_RESET}"
        return 0
    fi
