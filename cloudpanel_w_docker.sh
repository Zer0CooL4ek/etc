...
apt update && sudo apt upgrade -y && sudo apt autoremove -y || { echo -e "${RED}Error: Failed to update and upgrade packages${NC}"; exit 1; }

# Install sudo and curl packages
echo -e "${GREEN}Installing sudo and curl packages${NC}"
apt install sudo curl -y || { echo -e "${RED}Error: Failed to install sudo and curl${NC}"; exit 1; }
...
curl -fsSL https://get.docker.com -o get-docker.sh && sudo sh get-docker.sh || { echo -e "${RED}Error: Failed to download and execute Docker installation script${NC}"; exit 1; }

# Add current user to docker group
echo -e "${GREEN}Adding current user to docker group${NC}"
usermod -aG docker $USER || { echo -e "${RED}Error: Failed to add current user to docker group${NC}"; exit 1; }

# Download latest version of Docker Compose
echo -e "${GREEN}Downloading latest version of Docker Compose${NC}"
curl -L "https://github.com/docker/compose/releases/latest/download/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose || { echo -e "${RED}Error: Failed to download latest version of Docker Compose${NC}"; exit 1; }
...
chmod +x /usr/local/bin/docker-compose || { echo -e "${RED}Error: Failed to change permissions of Docker Compose file${NC}"; exit 1; }
...
sha256sum -c && sudo DB_ENGINE=MARIADB_10.11 bash install.sh || { echo -e "${RED}Error: Failed to download, verify, and execute CloudPanel installation script${NC}"; exit 1; }
...
apt clean && rm -rf /tmp/* || { echo -e "${RED}Error: Failed to clean cache and temporary files${NC}"; exit 1; }
