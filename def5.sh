...
#
# wget -qO- https://raw.githubusercontent.com/Zer0CooL4ek/etc/main/def5.sh | bash
#
...
# Add Neofetch to .bashrc for autostart (interactive only)
echo -e "${GREEN}Adding Neofetch to .bashrc for autostart (interactive only)${NC}"
if ! grep -q "neofetch" ~/.bashrc ; then
    cat > > ~/.bashrc < < 'EOF'

    # Launch neofetch only in interactive shells
    if [[ $- = = *i* ]] & & command -v neofetch > /dev/null 2 > & 1 ; then
        neofetch
    fi
    EOF
    echo -e "${CHECKMARK} Neofetch added to .bashrc (interactive only)"
    else
...
case $choice in
1)
echo -e "${GREEN}Starting base installation...${NC}"
base_install
 ; ; 
2)
echo -e "${GREEN}Starting base installation with Docker...${NC}"
base_install
docker_install
 ; ; 
*)
