...
#
# wget -qO- https://raw.githubusercontent.com/Zer0CooL4ek/etc/main/def6.sh | bash
#
...
# Add Neofetch to .bashrc for autostart (only in interactive sessions)
echo -e "${GREEN}Adding Neofetch to .bashrc for autostart${NC}"
if ! grep -q "neofetch" ~/.bashrc; then
    echo '[ -z "$PS1" ] || neofetch' >> ~/.bashrc
    echo -e "${CHECKMARK} Neofetch added to .bashrc for autostart (interactive sessions only)"
else
